...
class GamePopup:
    """Handles rendering for Win/Lose Popups (Encapsulation: Popup rendering)."""
    def __init__(self, is_win, difficulty):
        self.is_win = is_win
        self.difficulty = difficulty
        from constants import MATCH_COLOR, FAIL_COLOR, HIGHLIGHT_COLOR
    ...
    def draw(self, surface):
        ...
        p_rect = pygame.Rect(WIDTH//2-200, HEIGHT//2-160, 400, 360 if self.is_win and self.difficulty != "Hard" else 280)
        pygame.draw.rect(surface, (30, 30, 60), p_rect, border_radius=30)
        pygame.draw.rect(surface, MATCH_COLOR if self.is_win else FAIL_COLOR, p_rect, 4, border_radius=30)
        
        title_t = "YOU WIN!" if self.is_win else "YOU LOSE!"
        title_s = font_main.render(title_t, True, MATCH_COLOR if self.is_win else FAIL_COLOR)
        surface.blit(title_s, (WIDTH//2 - title_s.get_width()//2, HEIGHT//2 - 120))
        
        # Buttons definition
        btns = []
        if self.is_win:
            if self.difficulty != "Hard": btns.append(("NEXT LEVEL", MATCH_COLOR))
        ...
        btn_w, btn_h = 240, 55
        y_off = HEIGHT//2 + (0 if self.is_win and self.difficulty != "Hard" else 20)
...
class GameView:
    ...
    def draw(self, surface, game):
        ...
        if game.state in ["won", "failed"]:
            popup = GamePopup(game.state == "won", game.difficulty)
            popup.draw(surface)
